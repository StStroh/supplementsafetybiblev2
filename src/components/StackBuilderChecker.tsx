...
export default function StackBuilderChecker() {
  ...
  async function runCheck() {
    ...
    if (allItems.length < 2) {
      setError('Please add at least 2 items (supplements and/or medications)');
      return;
    }
  ...
  const canCheck = supplements.length + medications.length >= 2;
      ...
      <div className="text-center mb-8">
        ...
        {!canCheck && (
          <p className="text-sm mt-2" style={{ color: 'var(--color-text-muted)' }}>
            Add at least 2 items to run a check
          </p>
        )}
      </div>
        ...
        <div>
          ...
          <div className="rounded-xl p-6 mb-6" style={{ background: 'var(--color-surface)', border: '2px solid var(--color-border)' }}>
            ...
            <p className="text-base mb-4" style={{ color: 'var(--color-text-muted)' }}>
              Checked {summary.total_pairs} interaction{summary.total_pairs !== 1 ? 's' : ''} between your {supplements.length + medications.length} items.
            </p>
                        ...
                        <div className="flex items-start justify-between gap-4">
                          <div className="flex-1">
                            <h4 className="font-bold text-lg mb-1" style={{ color: config.textColor }}>
                              {getSubstanceName(result.a_substance_id)} + {getSubstanceName(result.b_substance_id)}
                            </h4>
